...
function carregarSessoes() {
    ...
    filme.salas.forEach(sala => {
        ...
        salaDiv.className = 'sessao-info';
        
        const horariosHtml = sala.horarios.map(horario => 
            `<button class="horario-btn" onclick="selecionarSessao(${sala.id}, '${sala.nome}', '${horario}')">${horario}</button>`
        ).join('');
...
function gerarPoltronas() {
    ...
    for (let fileira = 1; fileira <= 8; fileira++) {
        ...
        letraSpan.className = 'letra-fileira';
        ...
        for (let numero = 1; numero <= 10; numero++) {
            ...
            poltronaBtn.className = 'poltrona';
            ...
            if (poltronasOcupadas.includes(poltronaNumero)) {
                poltronaBtn.classList.add('ocupada');
...
function mostrarMensagem(mensagem, tipo) {
    ...
    div.className = `mensagem-sistema ${tipo}`;
